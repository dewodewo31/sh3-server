...
<div class="bg-gradient-to-br from-white/5 to-white/10 rounded-xl border border-white/10 p-8">
    ...
    <form action="{{ route('events.update', $event) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="category_id" 
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('category_id', $event->category_id) == $category->id ? 'selected' : '' }} class="text-black">
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <button type="button" onclick="addKeyPoint()" class="mt-2 text-green-400 hover:text-green-300 text-sm flex items-center gap-1 transition">
        ...
        @if($event->orders()->count() > 0)
            <div class="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4 mt-4">
                <div class="flex items-center gap-2">
                    ...
                    <p class="text-yellow-300 text-sm">Event ini sudah memiliki {{ $event->orders()->count() }} pesanan. Perubahan pada event akan mempengaruhi data yang sudah ada.</p>
                </div>
            </div>
        @endif
        ...
        <div class="flex gap-3 mt-6">
            ...
            <a href="{{ route('events.index') }}" class="flex-1 bg-gray-600 hover:bg-gray-700 text-white px-6 py-3 rounded-lg transition text-center">
                Batal
            </a>
        </div>
    </form>
