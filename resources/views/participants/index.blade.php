    ...
    <table class="w-full">
        <thead>
            <tr class="border-b border-white/10">
                ...
                <th class="text-left py-3 px-4 text-gray-400">Warning</th>
        ...
        <tbody>
            ...
            <tr class="border-b border-white/10 hover:bg-white/5 transition
                ...
                <td class="py-3 px-4">
                    <code class="
                        @if($participant->participant_type == 'member') text-purple-400 
                        @else text-green-400 @endif font-mono text-sm">
                        {{ $participant->hash_id }}
                    </code>
                </td>
                ...
                <td class="py-3 px-4">
                    <div class="flex items-center gap-3">
                        ...
                        <img src="{{ Storage::url($participant->photo) }}" class="w-8 h-8 rounded-full object-cover">
                        @else
                ...
                <!-- Type Member / Non-Member dengan warna berbeda -->
                <td class="py-3 px-4">
                ...
                <td class="py-3 px-4">
                    @if($participant->current_warning_level == 0)
                        <span class="px-2 py-1 bg-green-500/20 text-green-300 rounded-full text-xs flex items-center gap-1 w-fit">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            Clean
                    ...
                    @elseif($participant->current_warning_level == 1)
                        <span class="px-2 py-1 bg-yellow-500/20 text-yellow-300 rounded-full text-xs flex items-center gap-1 w-fit">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @elseif($participant->current_warning_level == 2)
                        <span class="px-2 py-1 bg-orange-500/20 text-orange-300 rounded-full text-xs flex items-center gap-1 w-fit">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                            </svg>
                            Warning 2
                    ...
                    @elseif($participant->current_warning_level >= 3)
                        <span class="px-2 py-1 bg-red-500/20 text-red-300 rounded-full text-xs flex items-center gap-1 w-fit">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                            </svg>
                            Suspended
                ...
                <td class="py-3 px-4">
                    <div class="flex items-center justify-center gap-2">
                        <a href="{{ route('participants.show', $participant) }}"
                            class="text-blue-400 hover:text-blue-300 transition" title="View Details">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <tr>
                <td colspan="9" class="py-8 text-center text-gray-400">
                    ...
                    @if(request()->hasAny(['search', 'type', 'status', 'gender', 'warning_level', 'sort']))
                        <p class="text-sm text-gray-500 mt-1">Try changing your filter criteria</p>
                        <a href="{{ route('participants.index') }}" class="text-green-400 hover:text-green-300 mt-2 inline-block">
                            Clear all filters
                        </a>
                    @else
